...
<script>
	...
	for(var i = 0; i < recentProjects.length; i++) {
		...
		item += '<div class="project__link">';
		item +=	'<a target="_blank" href="' + recentProjects[i].link + '">' + recentProjects[i].buttonText + '</a>';
		if( 'link2' in recentProjects[i] ) {
			console.log("true");
			item +=	'<a target="_blank" href="' + recentProjects[i].link2 + '">' + recentProjects[i].buttonText2 + '</a>';
		}
		item += '</div></div>';
	}
